...
public class Trident extends SubsystemBase {
 
    //This will make smartdashboardPuts goto the classes subfolder in the network tables. the / does the subfoldering.
  String className = this.getClass().getSimpleName()+"/";
  
  public final TalonFX m_TridentMotor = new TalonFX(constants.CanBus.TridentMotorCanID);

  private final com.ctre.phoenix6.controls.PositionDutyCycle m_positionDC = new PositionDutyCycle(0);
  private final com.ctre.phoenix6.controls.DutyCycleOut m_DutyCycle = new DutyCycleOut(constants.Trident.IntakeDutyCycle);
  ...
  private void setMotorConfig(){
    ...
    configuration.CurrentLimits.StatorCurrentLimit = constants.Trident.intakeAmpLimit;
  ...
  public void periodic() {
    ...
    public boolean isLoaded()
    {
        ...
        if(m_TridentMotor.getStatorCurrent().getValueAsDouble() > constants.Trident.intakeAmpCutoffThreshold)
        {   
            ...
            if(intakedebounceTimer.isRunning() && intakedebounceTimer.get() >= constants.Trident.intakeAmpLimittime)
            {
                alert.set(true);
